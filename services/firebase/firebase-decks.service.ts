import { addDoc, where, writeBatch } from 'firebase/firestore';
import {
	collection,
	doc,
	FIREBASE_AUTH,
...
export class FirebaseDecksService {
	...
	async getDecks(): Promise<Deck[]> {
		try {
			const decks: Deck[] = createDeckInstance(await this._getDecksRawDecks());
			...
			console.log('processed decks', decks);
			return decks;
		} catch (error) {
	...
	async _getDecksRawDecks(): Promise<DeckModel[]> {
		...
		try {
			...
			const decksWithCards: DeckModel[] = decks.map(({ id, data }) => {
				const pendingCardsResult = pendingCardsResults.find(
					(result) => result.deckId === id,
				);
				return {
					...data,
					id,
	...
	private _getDeckDocRef(deckId: string) {
		...
		return doc(FIREBASE_DB, 'users', currentUserId, COLLECTIONS.DECKS, deckId);
	}
}
